...
<style>
    ...
    :root {
        ...
        --accent: var(--color-primary, #00ffca);
    ...
    /* Anthropic Variable Fonts — served via Apollo /fonts/ */
    @font-face {
        font-family: 'Anthropic Sans';
        src: url('https://docs.bitandmortar.com/fonts/AnthropicSans-Romans-Variable-25x258.ttf') format('truetype');
        font-weight: 100 900;
        font-style: normal;
        font-display: swap;
    }
    @font-face {
        font-family: 'Anthropic Sans';
        src: url('https://docs.bitandmortar.com/fonts/AnthropicSans-Italics-Variable-25x258.ttf') format('truetype');
        font-weight: 100 900;
        font-style: italic;
        font-display: swap;
    }
    @font-face {
        ...
        src: url('https://docs.bitandmortar.com/fonts/AnthropicSerif-Romans-Variable-25x258.ttf') format('truetype');
        font-weight: 100 900;
        font-style: normal;
        font-display: swap;
    }
    @font-face {
        font-family: 'Anthropic Serif';
        src: url('https://docs.bitandmortar.com/fonts/AnthropicSerif-Italics-Variable-25x258.ttf') format('truetype');
        font-weight: 100 900;
        font-style: italic;
        font-display: swap;
    }
...
<link rel="stylesheet" href="/ui/aesthetics.css">
<script type="module">
    import { initAesthetic } from '/core/index.js';
    // Try to run in parent if we're in an iframe (Streamlit component)
    const target = window.parent === window ? window : window.parent;
    if (target && typeof target.initAesthetic !== 'function') {
        target.initAesthetic = initAesthetic;
        target.initAesthetic({ enableAmpersand: true });
    }
</script>
...
def main():
    ...
    st.markdown(f'''
        <div class="bm-header">
            <a href="/" target="_self" class="logo bm-title" style="text-decoration:none; color:inherit; display:flex; align-items:center; gap:0.5rem; font-weight:800; font-size:1.1rem; text-transform:lowercase;">
                bit<span id="bm-ampersand" style="color:var(--accent); font-style:normal; margin:0 2px;">&</span>mortar 
                <span class="bm-slash" style="opacity:0.4; margin:0 8px; font-family:monospace; font-size:0.8rem; color:var(--accent);">/</span> 
                <span class="bm-sub" style="font-style:italic; font-family:serif; font-weight:400;">resume_builder</span>
            </a>
            <div style="font-family: 'Anthropic Sans'; font-size: 0.7rem; color: var(--dim); letter-spacing: 0.2em; text-transform: uppercase;">NEURAL MIRROR // LOCAL-FIRST INTELLIGENCE</div>
        </div>
    ''', unsafe_allow_html=True)
    ...
    if not st.session_state.generation_result:
        with st.container():
            st.file_uploader("Drop CV/Knowledge pieces here", type=["pdf", "md", "docx"], label_visibility="collapsed", accept_multiple_files=True)
